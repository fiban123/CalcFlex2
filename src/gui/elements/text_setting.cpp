...
void TextSetting::update_click(sf::Vector2i mouse_pos) {
    if (rect_point_collide(mouse_pos, rect)) {
        if (input.selected != true) {
            input.selected = true;
            input.text_area.style =
                &style->input_style.selected_text_area_style;
            input.text_area.update_style();
        }
    }
    else {
        if (input.selected != false) {
            input.text_area.style =
                &style->input_style.text_area_style;
            input.text_area.update_style();
...
TextSetting::TextSetting(std::string& label_string,
    ...
    label = TextArea(label_string,
                     pos,
                     {0, style->label_height},
                     &style->label_style,
                     XBound::CENTER,
                     YBound::CENTER);
    ...
    if (!label.text_lines.empty()) {
        label_max_len =
            std::max(label_max_len,
                     label.text_lines[0].getLocalBounds().width);
    }

    label_max_len = label.text_lines[0].getLocalBounds().width;
    label.resize(
        {label_max_len + 2 * style->spacing, style->label_height});

    // button and input
    sf::Vector2f button_pos{
        pos.x + style->label_style.spacing + label.rect.getSize().x,
        pos.y};
    info_button =
        HoverButton(button_pos,
                    {style->label_height, style->label_height},
                    "i",
                    &style->info_button_style);

    sf::Vector2f input_pos{
        pos.x, pos.y + style->label_height + style->spacing};

    input = TextInput(setting_str,
                      input_pos,
                      {text_input_width, style->text_input_height},
                      &style->input_style,
                      _selected,
                      XBound::LEFT,
                      YBound::CENTER);
    ...
    sf::Vector2f info_pos{button_pos.x + style->label_height,
                          button_pos.y};

    unsigned newline_count = std::ranges::count(info_str, '\n');

    float info_height =
        (1 + newline_count) * (style->info_style.font_size +
                               style->info_style.entry_spacing);

    info = TextArea(info_str,
                    info_pos,
                    {0, info_height},
                    &style->info_style,
                    XBound::CENTER,
    ...
    if (!info.text_lines.empty()) {
        info_max_len = std::max(
            info_max_len, info.text_lines[0].getLocalBounds().width);
    }
    ...
    rect.setSize(
        {std::max(label_max_len + style->label_style.spacing +
                      style->label_height,
                  text_input_width),
         style->label_height + style->spacing +
             style->text_input_height});
}
